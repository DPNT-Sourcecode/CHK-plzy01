...
def process_type3_offers(item_count: dict, item_price_map: dict) -> tuple[dict, int]:
    ...
    for offer in specials[OfferTypes.TYPE_3]:
        ...
        discount_items_price_map = {item: price for item, price in item_price_map.items() if item in offer.items}
        sorted_item_price_map = dict(sorted(discount_items_price_map.items(), key=lambda item: item[1], reverse=True))
